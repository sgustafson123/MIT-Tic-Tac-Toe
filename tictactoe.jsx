const Board = () => {
  ...
  let status = `Vinnare ar: ${checkForWinner(gameState)}`;

  // Part 1 step 1 code goes here
  // Use conditional logic to set a variable to either 'Player O' or  'Player X'
  let playerTurn = `Nasta spelare: ${
    player == "O" ? "Spelare O" : "Spelare X"
  }`;
  console.log(`Vi har en vinnare ${status}`);
  ...
  return (
    <button
      ...
      onClick={() => {
        setTik(takeTurn(id));
        setFilled(true);
        console.log(`Fyrkant: ${id} fylld av spellare : ${tik}`);
      }}
    >
      <h1>{mark[tik]}</h1>
    </button>
...
const checkForWinner = (gameState) => {
  ...
  if (gameState.length < 5) return "Ingen vinnare an.";
  ...
  if (win0.length > 0) return "Spellare O ";
  else if (winX.length > 0) return "Spellare X ";
